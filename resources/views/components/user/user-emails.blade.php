...
<div class="flex items-center justify-between border-b pb-4">
    ...
    <input type="text" id="emailId" class="text-[21px] font-semibold border border-gray-300 rounded px-2 py-1" value="{{$item->email_name ?? ''}}" required
/>
    ...
    <div class="w-full pl-3 leading-normal lg:w-7/12 lg:pr-4">
        <div class="border-b pb-4">
            <p class="font-semibold text-sm">
                ...
                <span id="subject" class="font-normal text-sm px-1" value="{{$item->subject ?? ''}}" required>
                    <!-- {{ 'Potential for expansion abroad' }} -->
                </span>
            </p>
        </div>

        <p id="description" class="mt-2 pb-3 px-1 text-sm leading-normal"  value="{{$item->description ?? ''}}" required >
