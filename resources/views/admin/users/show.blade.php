        ...
        <div class="flex items-center gap-4">
            <form action="{{ route('users.importCsv', $user->id) }}" method="post" enctype="multipart/form-data"
                ...
                <div class="border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 flex items-center">
                    <input type="file" name="excel_file" id="excel_file" class="text-sm text-gray-700" {{ $compaigns->isEmpty() ? 'disabled' : '' }}>
                </div>

                <button type="submit"
                    class="bg-purple-500 text-white px-6 py-1.5 rounded-md flex items-center text-sm {{ $compaigns->isEmpty() ? 'opacity-50 cursor-not-allowed' : '' }}"
                    {{ $compaigns->isEmpty() ? 'disabled' : '' }}>
            ...
            <div class="flex items-center gap-[13px] md:w-[359px] md:h-[35px]">
                <div>
                    <div class="flex flex-row gap-1 border border-gray-300 rounded-lg px-3 py-2 bg-white h-[40px]">
                        ...
                        <div>
                            <select name="campaign_id" id="campaignId" class="rounded-lg w-25 focus:outline-none overflow-hiddenc cursor-pointer">
...
<script>
    function fetchLeads(page = 1) {
        ...
        let userId = '{{ $user->id }}'
        let actionUrl = `{{ route('users.show', ':id') }}`;
